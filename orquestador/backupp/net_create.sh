...
if [ -z "$ID" ] || [ -z "$VLANS" ] || [ -z "$OVS_BRIDGE" ]; then
    echo "ERROR: Faltan parámetros"
    echo "Uso: $0 <id> <vlans> <ovs_bridge>"
    echo "Ejemplo: $0 test1 1;4 br-cloud"
    exit 1
fi
...
echo "Primera VLAN: $START_VLAN (subnet: $FIRST_VLAN_SUBNET.0/24, gateway: $FIRST_VLAN_SUBNET.1)"
echo "Gateway automático: Cada VLAN usa {subnet}.1 como gateway"
echo ""
...
for VLAN_ID in $(seq "$START_VLAN" "$END_VLAN"); do
    ...
    # Todas las VLANs usan el .1 de su subnet como gateway
    ...
    echo "  - Puerto interno OVS creado"
