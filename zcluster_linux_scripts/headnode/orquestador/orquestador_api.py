...
async def startup_event():
    """Inicializar VNC Manager al arrancar la aplicación"""
    global vnc_manager
    try:
...
async def desplegar_slice(
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"✗ DESPLIEGUE FALLIDO después de {len(all_attempts)} intentos")
        print(f"{'='*60}")
...
async def eliminar_slice(
    ...
    try:
        ...
        if vnc_released:
            ...
            print(f"   ⚠ No se encontraron puertos VNC para liberar (slice no existía o ya liberado)")
